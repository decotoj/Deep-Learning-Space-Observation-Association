...
def bins_labels(bins, **kwargs):
    ...
    plt.xticks(
        np.arange(min(bins) + bin_w / 2, max(bins), bin_w), bins, **kwargs
    )
...
for i in range(len(obs)):
    ...
    for j in range(len(obs)):
        if i != j:
            ...
            datapoint = trnMod.augmentAndVectorize(
                [obs[0][:], obs[-1][:]]
            )  # dummy
...
ax.grid()
ax.set(
    xlabel="Truth Number of RSOs in Triplet",
    ylabel="# Occurances",
    title="UCS Observation Triplet Association Results",
)
bins = [1, 2, 3, 4]
