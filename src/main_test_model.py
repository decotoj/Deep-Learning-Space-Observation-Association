    ...
    for j in range(len(obs)):
        ...
        if i != j:
            d = [
                obs[i][:],
                obs[j][:],
            ]  # datapoint consisting of pair of observations
...
ax.set(
    ...
    title="Model Applied to Sparsely Matched Test Data",
)
ax.grid()
...
text = (
    ...
    + "Probability of Pair Match: "
    + str(round(len(p1) / (len(p1) + len(p0)), 4))
)
...
ax.set(
    xlabel="Model Score",
    ylabel="Distribution",
    title="Distribution of Test Data Pairs",
)
ax.grid()
