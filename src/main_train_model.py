...
learning_rate = 1e-4  # Baseline = 1e-4
        ...
        for q in [4, 5, 6, 7, 8, 9, 10]:
            for r in [4, 5, 6, 7, 8, 9, 10]:
                for s in [0, 4, 5, 6, 7, 8, 9, 10]:
                    ob[z].append(
                        (ob[z][q] - ob[z - 1][r]) / (ob[z][s] - ob[z - 1][s])
                    )
...
def defineModel():
    ...
    model = torch.nn.Sequential(
        ...
        torch.nn.LogSoftmax(
            1
        ),  # 11/14/2019: Changed to (1) instead of () due to depracation warning, untested
    ).to(device)
...
if __name__ == "__main__":
    ...
    if TRAIN_FLAG >= 1:
        ...
        for t in range(NUMEPOCHS):
            ...
            model.eval()
            acc_val = accuracy(
                y_pred_val.detach().cpu().numpy(), y_val.detach().cpu().numpy()
            )
            model.train()
            print(
                t,
